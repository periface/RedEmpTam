...
namespace MercadoCinotam.Tests
{
    public abstract class MercadoCinotamTestBase : AbpIntegratedTestBase
    {
        ...
        protected override void AddModules(ITypeList<AbpModule> modules)
        {
            ...
            modules.Add<MercadoCinotamTestModule>();
        }
        ...
        protected void UsingDbContext(Action<MercadoCinotamDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<MercadoCinotamDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MercadoCinotamDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MercadoCinotamDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MercadoCinotamDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MercadoCinotamDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<MercadoCinotamDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MercadoCinotamDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MercadoCinotamDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MercadoCinotamDbContext, Task<T>> func)
        {
            T result;
